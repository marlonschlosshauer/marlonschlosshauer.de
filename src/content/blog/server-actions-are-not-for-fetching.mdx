...
While we already have plenty of options, I think this showcases that a "server query" could be a great addition. Especially now with the new `"use cache"` semantic. Let's see what the future may bring.
...
<Note.Frame variant="secondary">
    ...
    <Note.Body>
        TanStack Start provides [Server
        Functions](https://tanstack.com/start/latest/docs/framework/react/guide/server-functions#static-server-functions)
        which can be used to query data. This is especially cool because it lets [you grab RSC from
        the client](https://tanstack.com/blog/react-server-components#what-do-they-look-like)
        without any of the drawbacks mentioned in this post!
    </Note.Body>
</Note.Frame>
