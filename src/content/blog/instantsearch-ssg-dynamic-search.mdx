...
export const metadata = {
    title: "Optimizing InstantSearch for SSG (Deprecated)",
    description:
...
# Optimizing InstantSearch for SSG (Deprecated)
...
<Note.Frame variant="secondary">
    ...
    <Note.Body>
        This approach ended up not shipping as it had many subtle issues with Next.js (particularly
        with navigation). Instead of continuing with <code>react-instantsearch-nextjs</code> we
        decided to throw Claude at the problem; creating a version that mirrors the surface API of
        InstantSearch but works in a React-idiomatic way. That ended up shipping and was a complete
        success!
    </Note.Body>
</Note.Frame>
