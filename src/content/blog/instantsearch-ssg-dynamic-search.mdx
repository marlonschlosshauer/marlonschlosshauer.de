...
1. No children inside of InstantSearch were server-rendered, resulting in a huge layout shift
2. Each visitor would always do a request against Algolia

While problem 1. can be circumvented with hacks, it really was the insane costs from problem 2. that made me want to dive deeper. Ideally our solution should:
...
```
    useEffect(() => {
        ...
        if (!canReadRealLocation) {
            // Enable reading from real URL
            canReadRealLocation = true;
...
As you can see we now use the `canReadRealLocation` flag. If it has not been set, we set it after hydration. Then we dispatch a `PopStateEvent` to notify InstantSearch that we want to search, however we only do so if our URL has search params. Afterwards we schedule our `canPush` flag to be set to `true` after the render is completed, so after that InstantSearch can touch the URL.
