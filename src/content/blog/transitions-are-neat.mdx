...
The Concurrent API hasn't revolutionized the way I write components but I'm always delighted when I get to use one of its tools. While building a new feature I found a neat interaction with the `useTransition` hook and I wanted to document it here.

We're going to explore the interplay between conditional rendering, `useTransition` and Suspense.
...
In fact, as I understand it, these two features are complementary to each other! What do I mean by this? Well: We can't wrap a `<Suspense>` around our transition (because transitions aren't Suspense-enabled). But what happens if we do the opposite?
...
Let's say our form shows a component conditionally: e.g. if a user selects a country, a note should appear. Let's go a step further: That note should only be shown sometimes and the only way to find out, is to do call a server. In the meantime we cannot allow the user to submit the form without the loading being completed.

Usually we would move our fetch call up (into our parent) & render it conditionally. That way both our fetch and button are (at least) at the same level, thus allowing us to show a loading spinner and controlling the submit button. Our code would look like this:
...
This form is interesting because it moved the fetch into the note. And it uses the `useTransition` hook to capture the loading state. The following is happening:

1. We conditionally render the note
2. The note always fetches, to check if it should be shown
3. If it should be shown, the note shows a message (otherwise it exits early)
4. Our setting of state is marked as a _transition_
...
Would a rogue `await` alongside my `useState` really be the end of the world?
