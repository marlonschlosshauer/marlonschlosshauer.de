...
1. Create a bunch of `.json` dictionaries somewhere
...
2. Create a basic translation provider that loads the dictionaries
...
4. Add a hook that returns a translation function, which uses the dictionary from our provider
...
5. Create a `layout.tsx` in `[locale]` and mount the provider
