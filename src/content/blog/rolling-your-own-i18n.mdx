...
export const metadata = {
    title: "Rolling your own i18n in Next.js",
    description:
        "Building type safe, optimized, i18n is simple, fun and saves you from hidden magic.",
    date: "2025-10-30",
    words: 1800,
...
You can find the code for all the examples [here](https://github.com/marlonschlosshauer/next.js-roll-your-own-i18n-demo).
...
With some minor refactoring we translate on the server, too! Let's clean up first by moving all of the shared stuff into a `shared.ts`:
...
First let's start by creating a `type.ts` file where we move the types from `shared.ts`:
...
Now let's move the `getDictionary()` function into `server.ts`:
...
With this I can now place templates into my translation strings and replace them afterwards, during translation:
