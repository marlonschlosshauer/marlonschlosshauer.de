...
1. Redirecting user based on location: Instead of checking the header in your happy path, first redirect the user to a dedicated redirect route, then you can safely check the header there without deopting your route.
2. Auth check in header: Instead of calling header in your navigation to see if the user is logged in or not, show a fallback on the server and check that on the client via an additional fetch
